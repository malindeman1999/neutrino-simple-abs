...
def compute_imag_alpha_vs_tc(percent_tc):
    ...
    for tc_k in tc_values_k:
        ...
        t_eval_k = (percent_tc / 100.0) * tc_k
    ...
    return tc_values_k, imag_thick, imag_extreme, imag_thin


def main():
    percent_tc =  10.0
    print(f"Processing {percent_tc:g}% Tc")
    tc_values_k, imag_thick, imag_extreme, imag_thin = compute_imag_alpha_vs_tc(percent_tc)
    ...
    plt.xlabel("Tc (K)")
    plt.ylabel(f"Imaginary Alpha at T = {percent_tc:g}% Tc")
    plt.title(f"Imaginary Alpha vs Tc (Exact, T = {percent_tc:g}% Tc)")
